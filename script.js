...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!titleInput.validity.valid) {
      titleError.textContent = "Invalid entry";
      if (titleInput.validity.valueMissing) {
        titleError.textContent = "Must enter at least 1 character";
      }
    }
    if (!authorInput.validity.valid) {
      if (authorInput.validity.valueMissing) {
        authorError.textContent = "Must enter at least 1 character";
      }
    }
    if (!pagesInput.validity.valid) {
      if (pagesInput.validity.rangeUnderflow) {
        pagesError.textContent = "Must have at least 1 page";
      }
      if (pagesInput.validity.rangeOverflow) {
        pagesError.textContent = "Can't be more than 9999 pages";
      }
      if (pagesInput.validity.valueMissing) {
        pagesError.textContent = "Enter a number between 1 and 9999";
      }
    }
